...
# COMPILE-2017.command by Bob Maple
# Compiles and installs Matchbox and Lightbox shaders from http://logik-matchbook.org/ into IFFFS 2017 versions
# Place in the same directory of the download as INSTALL.command and run instead.

# shader_builder in the 2017 release has obtained some interesting new behaviors;
# If try to compile any shader from any directory other than the current working directory,
# the resulting .mx will have no thumbnail.  Therefore one change from the 2016 version of
# this script changes the CWD to the shaders/ dir so it can build directly from there.
